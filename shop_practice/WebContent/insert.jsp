...
<script>
	function joinCheck(){//유효성 검사 진행
		if(document.join.custname.value.replace(/\s/gi,"")==""){
			alert("회원 이름을 입력해주세요.");
			document.join,custname.focus();
			return false;
		}
		if(document.join.phone.value.replace(/\s/gi,"")==""){
			alert("회원전화를 입력해주세요.");
			document.join,phone.focus();
			return false;
		}
		if(document.join.address.value.replace(/\s/gi,"")==""){
			alert("회원주소를 입력해주세요.");
			document.join,address.focus();
			return false;
		}
		if(document.join.joindate.value.replace(/\s/gi,"")==""){
			alert("가입일자를 입력해주세요.");
			document.join,joindate.focus();
			return false;
		}
		if(document.join.grade.value.replace(/\s/gi,"")==""){
			alert("고객등급을 입력해주세요.");
			document.join,grade.focus();
			return false;
		}
		if(document.join.city.value.replace(/\s/gi,"")==""){
			alert("도시코드를 입력해주세요.");
